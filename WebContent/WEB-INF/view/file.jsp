...
<body>
	...
	<table>
		...
		<c:if test="${requestScope.fatherFolder.id != null }">
			<tr>
				<td>
					<a href="/cloud/main/file?dir=${requestScope.fatherFolder.id }"><img src="/cloud/images/folder.png" alt="folder" /></a>
				</td>
				<td>上级文件夹</td>
				<td>文件夹</td>
				<td></td>
		...
		<c:forEach var="list" items="${requestScope.fileList }">
			<tr>
				...
				<c:if test="${list.status == 0}">
					<td>
						<a href="setAccess?file=${list.id }&folder=${requestScope.folderId}"><img src="/cloud/images/private.png" alt="status" /></a>
					</td>
				</c:if>
				<c:if test="${list.status == 1}">
					<td>
						<a href="setAccess?file=${list.id }&folder=${requestScope.folderId}"><img src="/cloud/images/public.png" alt="status" /></a>
					</td>
				</c:if>
